...
const sccCheckList_templatePath =
  "./tmp/scc_attendance_check_list_template.docx";
...
var numberOfweeksEachYear = [53,52,52,52,52,52,53,52,52,52,52]; //from 2020 to 2030
var dateArray;
  ...
  for (let i = 0; i < forms.length; ++i) {
    ...
    if (forms[i].includes(collection_resultPath.split("/")[3])) {
      carbone.render(collection_templatePath, date, function(err, result) {
        ...
        console.log(
          "server generated collection form with date " + date.fullDate
        );
      });
    }
    if (forms[i].includes(stockCheckList_resultPath.split("/")[3])) {
      carbone.render(stockCheckList_templatePath, date, function(err, result) {
        ...
        console.log(
          "server generated stock check list with date " + date.fullDate
        );
      });
    }
    if (forms[i].includes(sccCheckList_resultPath.split("/")[3])) {
      carbone.render(sccCheckList_templatePath, date, function(err, result) {
        ...
        console.log(
          "server generated ssc attendance check list with date " +
            date.fullDate
        );
      });
    }
...
function findClosetPH() {}

function checkPH(date) {
  for (let i = 0; i < Object.keys(phs).length; ++i) {
    if (
      phs[i]["date"] == date.getDate() &&
      phs[i]["month"] == months[date.getMonth()]
    ) {
      return true;
    }
  }
...
function dateSetting(date) {
  ...
  if (leapYear(syear))
    datesOfMonths = [31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
  ...
  //accumated from year 2020
  for (let i = 0; i < (syear-2020); i++) 
   {     
     numOfWeek+=numberOfweeksEachYear[i];
   }  
}
...
function mappingTheRule(rule) {
  if (rule.includes("ph")) {
    if (isPh || sweekday == 0) return true;
  }
  ...
  if (rule.includes("year")) {
    ...
    for (let k = 0; k < yearlyRules.length; ++k) {
      ...
      console.log(yearlydate[0]);
      console.log(yearlydate[1]);
...
function filteringForTaskScheduler(data, first_loaded) {
  ...
  for (let i = 0; i < numberOfData; ++i) {
    ...
    if (include.substring(0, 5) == "daily") {
      ...
      //Eliminating the selected one-off Tasks (Holtin: to be followed up)
    ...
    if (include == "SC3") {
      ...
      if (sweekday == 1) {
        var thisMonthLastSunday = lastSundayOfSelectedMonth(
          selectDate.getMonth()
        );
        var lastMonthLastSunday = lastSundayOfSelectedMonth(
        ...
        if (
          selectDate.getDate() == thisMonthLastSunday.getDate() ||
          (selectDate.getDate() == lastMonthLastSunday.getDate() &&
            selectDate.getMonth() == lastMonthLastSunday.getMonth())
        ) {
          metTheRule = true;
        }
      }
    ...
    if (metTheRule && task["Rules"][task["Rules"].length - 1] == "@") {
      var OOJson = fs.readFileSync(oneOffPath, "utf8");
      OOJson = JSON.parse(OOJson);
      OOJson.splice(i, 1);
      fs.writeFileSync(oneOffPath, JSON.stringify(OOJson), "utf8");
...
function tapeLocationForWeekly(
  task,
  numberOfTapesSet,
  destination,
  offset,
  ...
  if (destination == "SCC") {
    ...
    for (var k = 0; k < numberOfTapesSet; k++) {
      if (SysName == "V5") {
        ...
        if (tapeSent == k) tapesInSCC.push(task["Tapes"][k]);
        else tapesInPCC.push(task["Tapes"][k]);
      } else {
        //for PPS and VRMS,only tapeBack and next week tape in PCC, the rest are in SCC
        if (k == tapeBack || k == tapeNext) tapesInPCC.push(task["Tapes"][k]);
        else tapesInSCC.push(task["Tapes"][k]);
      }
    }
...
function tapeLocationForMonthly(
  task,
  numberOfTapesSet,
  destination,
  offset,
  ...
  if (SysName == "V5") {
    ...
    if (sday <= 7) {
      ...
      if (sday - weekday < 1) {
        ...
        base = base - 1;
        if (base == -1) base = numberOfTapesSet - 1;
      }
    }
  } else {
...
function tapelocationArrangment(
  system,
  moffset,
  mboffset,
  woffset,
...
function prepareTapeLocation() {
  ...
  tapelocationArrangment(pps, 0, 1, 1, 2, "PPS");
}
